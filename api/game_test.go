...
func TestStartGame_Success(t *testing.T) {
	...
	// Check the response body.
	expected := `{"success":true,"status":200,"result":{"name":"teste","rows":2,"cols":2,"mines":4,"status":"started","grid":[[{"mine":false,"clicked":false,"value":1},{"mine":false,"clicked":false,"value":1}],[{"mine":false,"clicked":false,"value":1},{"mine":true,"clicked":false,"value":0}]]}}`
	if !strings.Contains(rr.Body.String(), expected) {
		t.Errorf("handler returned unexpected body: want %v, got %v",
			expected, rr.Body.String())
	}
